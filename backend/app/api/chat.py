...
from app.models.chat import Chat, ChatCreate, Message, MessageCreate, ChatWithMessages, ChatResponse
...
async def create_chat(
    ...
    # Créer le chat
    await client.index(settings.CHAT_INDEX).add_documents([chat_dict])
...
async def list_chats(current_user: User = Depends(get_current_active_user)):
    ...
    result = await client.index(settings.CHAT_INDEX).search(filter=f"user_id = {current_user.id}", sort=["created_at:desc"])
...
async def get_chat(
    ...
    # Récupérer le chat
    chat_result = await client.index(settings.CHAT_INDEX).search(filter=f"id = {chat_id} AND user_id = {current_user.id}", limit=1)
    ...
    messages_result = await client.index(settings.MESSAGE_INDEX).search(filter=f"chat_id = {chat_id}", sort=["created_at:asc"], limit=100)
