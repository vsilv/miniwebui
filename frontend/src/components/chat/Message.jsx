...
const Message = ({ message }) => {
  ...
  const regenerateResponse = async () => {
    ...
    try {
      ...
      const updatedMessages = messages.filter((_, index) => index < assistantIndex);
      ...
      try {
        ...
        const sessionResponse = await api.post(`chat/${chatId}/messages/stream`, {
          json: {
            role: "user",
            content: userMessage.content // Use original user message content
          }
        }).json();
        ...
        eventSource.onmessage = (event) => {
          try {
            ...
            const messageIndex = currentMessages.findIndex(msg => msg.id === message_id);
        ...
        eventSource.onerror = (error) => {
          ...
          if (messageIndex !== -1) {
            const updatedMessage = {
              ...
              error: true,
              is_streaming: false,
              content: currentMessages[messageIndex].content || "Une erreur de connexion s'est produite"
            };
          ...
          {isUser ? (
            ...
            <div className="flex flex-col">
              <div className="flex items-start mb-2">
                ...
                <div>
                  ...
                  {message.content ? (
                    ...
                    <p>La génération de la réponse a rencontré un problème. Veuillez cliquer sur le bouton de régénération pour réessayer.</p>
                  )}
                </div>
              </div>
              <button 
              ...
              >
                ...
                <span>{isRegenerating ? 'Régénération...' : 'Régénérer la réponse'}</span>
              </button>
            </div>
          ) : (
            <MarkdownContent content={message.content} />
            ...
            {!isUser && !isStreaming && !hasError && (
              <div className="ml-4 flex items-center space-x-2 opacity-100 transition-opacity">
                ...
                <button
                  onClick={regenerateResponse}
                  disabled={isRegenerating}
                  className={`p-1 hover:text-gray-700 dark:hover:text-gray-300 rounded-full ${isRegenerating ? 'opacity-50 cursor-not-allowed' : ''}`}
