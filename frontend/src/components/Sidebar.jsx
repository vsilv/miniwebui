...
import { 
  PlusCircle, MessageCircle, BookOpen, Trash2, 
  Edit2, X, Search, ChevronRight, ChevronDown 
} from 'lucide-react';
...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside 
        className={`sidebar fixed md:static inset-y-0 left-0 z-30 w-64 bg-gray-50 dark:bg-dark-800 shadow-md transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col`}
      >
        {/* Sidebar header */}
                  ...
                  filteredChats.map((chat) => (
                    <div key={chat.id} className="relative group">
                      {editingChatId === chat.id ? (
                        <form onSubmit={updateChatTitle} className="flex items-center">
                          <input
                            type="text"
                            value={newChatTitle}
                        ...
                        >
                          ...
                          <div className="flex-shrink-0 flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              onClick={(e) => startEditChat(chat.id, chat.title, e)}
